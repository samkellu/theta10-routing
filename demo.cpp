...
cone low_angle_alg(SDL_Renderer* renderer, vec2 cur_point, cone* cones, int n_cones, cone found_cone) {
...
void route(SDL_Renderer* renderer) {
	...
	while (max_steps-- >= 0) {
		...
		cone best_cone = low_angle_alg(renderer, cur_point, cones, n_cones, found_cone);
